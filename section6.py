...
def main():
    # Define the two lists of coordinates
    coordinates1 = (
    ...
    foreground = Image.new('RGBA',
                           (max(coordinates1[::2] + coordinates2[::2]), max(coordinates1[1::2] + coordinates2[1::2])))

    # Create a drawing object
    draw = ImageDraw.Draw(foreground)

    # Draw the lines between the coordinates
    draw.line(coordinates1, fill='black', width=2)
    draw.line(coordinates2, fill='black', width=2)
    ...
    # Create a new image with the same size as the foreground image
    combined_image = Image.new('RGBA', foreground.size)

    # Paste the background image onto the new image at (0, 0) position
    combined_image.paste(background, (0, 0))

    # Paste the foreground image onto the new image, preserving transparency
    combined_image = Image.alpha_composite(combined_image.convert('RGBA'), foreground.convert('RGBA'))
